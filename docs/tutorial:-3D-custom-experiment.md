...
For the post-processing, it is possible to use prepared `pyCtrlScripts/runBread3DOurExp.py` script, which generates the left-hand side part of the following figure. The right-hand side can be generated by `pyCtrlScripts/runBread2DOurExpFreeBread.py` with the axisymmetric 2D geometry.

<img alt="tutBread3Dcomp2D" src="tutBread3Dcomp2D.png" />

### Paraview post-processing
To further examine the results, you can visualize them e.g. utilizing prepared paraview state `ZZ_dataForPostProcessing/seeTomegaV3D.pvsm`.

<img alt="tutBreadSee3DTOmegaV" src="tutBreadSee3DTOmegaV.png" />
