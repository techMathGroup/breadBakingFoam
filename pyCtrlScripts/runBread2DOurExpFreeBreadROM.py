...
from prepareBakingCurve import createBakingCurve
import re
...
def runSim(outFolderDir, pars):
    ...
    plusTime1 = 2400 # -- how long to run with deformation
    plusTime2 = 0 # -- how long to run without deformation
    writeInt = 60   # -- how often to write results
    ...
    if prepBlockMesh:
        prep3DMeshOurExp(rLoaf1, rLoaf2, hLoaf, dX, dY, dZ, grX, grY, grZ, baseCase, for2DExtrude=True, up=up)
    ...
    # -- prepare baking curve
    time, bakingCurve = createBakingCurve(pars)

    # -- add baking curve to the dictionary
    # baseCase.replace(
    #     [
    #         ['0.org/T', ['TInfTable'], ['TInfTableBakingCurve']],
    #     ]
    # )
    ...
    with open(os.path.join(baseCase.dir, "constant", "TInfTable"), "w") as fl:
        ...
        for i in range(len(time)):
            fl.write("\t(%.5g\t%.5g)\n"%(time[i]*60, bakingCurve[i] + 273.15))
        fl.writelines(")\n")
