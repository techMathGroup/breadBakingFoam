...
def saveFigPostProcess(simDir):
    ...
    tUnified = np.linspace(0, 20,100)
    Ts = np.copy(tUnified.reshape(-1,1))

    for j in range(len(experiments)):
    # for i in range(4):
        # expTemp = np.loadtxt(os.path.join(expDir, '%d.dat'%j), delimiter='\t', skiprows=1)
        expTemp = np.loadtxt(os.path.join(expDir, os.path.join(expDir, experiments[j][0], experiments[j][1])), delimiter='\t', skiprows=1)

        expTemp = expTemp[expTemp[:, 1] >= 50]
        ...
        Ts = np.append(Ts, tHere.reshape(-1,1), axis=1)
    ...
    mean = np.mean(Ts[:,1:], axis=1)
    std = np.std(Ts[:,1:], axis=1)
    Ts = np.append(Ts, mean.reshape(-1,1), axis=1)
    Ts = np.append(Ts, std.reshape(-1,1), axis=1)

    np.savetxt("latex_data/expTemps.dat", Ts, header="Time(min)\tExp1\tExp2\tExp3\tExp4\tExp5\tExp6\tExp7\tExp8\tMean\tStd", comments='', fmt='%.6f', delimiter='\t')
    ...
    #     save_for_latex(os.path.join(out_dir, f'temp_exp_TC{j+1}.dat'), x_exp, y_exp, "Time(min)\tTemp(C)")
        ...
        # Simulation
    x_sim = simData[f'sim1'][:,0] / 60 - kynuti / 60
    y_sim = simData[f'sim1'][:,1] - 273
    ax1.plot(x_sim, y_sim, '--', color=colorLst[0], linewidth=2, label=f'Sim TC1')

    x_sim = simData[f'sim2'][:,0] / 60 - kynuti / 60
    y_sim = simData[f'sim2'][:,1] - 273
    ax2.plot(x_sim, y_sim, '--', color=colorLst[1], linewidth=2, label=f'Sim TC2')

    x_sim = simData[f'sim3'][:,0] / 60 - kynuti / 60
    y_sim = simData[f'sim3'][:,1] - 273
    ax3.plot(x_sim, y_sim, '--', color=colorLst[2], linewidth=2, label=f'Sim TC3')

    x_sim = simData[f'sim4'][:,0] / 60 - kynuti / 60
    y_sim = simData[f'sim4'][:,1] - 273
    ax4.plot(x_sim, y_sim, '--', color=colorLst[3], linewidth=2, label=f'Sim TC4')
...
simDir = '../ZZ_cases/2026/ELISPA1/exp0_nonDef_False/osatkaV2/'
